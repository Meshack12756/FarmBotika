...
class ProtectedView(APIView):
    ...
    def get(self, request):
        return Response({"detail": f"Hello, {request.user.username}"})
...
# 📬 Email verification sender (safe)
def send_verification_email(request, user):
    try:
        token = default_token_generator.make_token(user)
        verify_path = reverse('verify-email')
        verify_url = request.build_absolute_uri(f"{verify_path}?uid={user.id}&token={token}")
...
class LoginHistoryView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return self.request.user.logins.order_by('-timestamp')
